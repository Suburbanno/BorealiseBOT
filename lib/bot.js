...
export class BorealiseBot {
  ...
  async stop() {
    if (this._pipeline) {
      ...
      for (const timer of this._deleteTimers) {
        clearTimeout(timer);
      }
  ...
  _startPipeline() {
    ...
    this._pipeline.onConnection("onReady", (ready) => {
      ...
      const coreEvents = [
        ...
        Events.ROOM_CHAT_DELETE,
        Events.ROOM_CHAT_UPDATE,
        Events.ROOM_DJ_ADVANCE,
        Events.ROOM_WAITLIST_JOIN,
        Events.ROOM_WAITLIST_LEAVE,
        Events.ROOM_WAITLIST_UPDATE,
        Events.ROOM_WAITLIST_LOCK,
        Events.ROOM_WAITLIST_CYCLE,
        Events.ROOM_TIME_SYNC,
        Events.ROOM_VOTE,
        Events.ROOM_GRAB,
        ...
        Events.ROOM_USER_MUTE,
        Events.ROOM_USER_UNMUTE,
        Events.ROOM_USER_ROLE_UPDATE,
        Events.ROOM_USER_AVATAR_UPDATE,
        Events.ROOM_USER_SUBSCRIPTION_UPDATE,
    ...
    this._pipeline.on(Events.ROOM_WAITLIST_JOIN, (data) =>
      this._onWaitlistJoin(data),
    );
    ...
    this._pipeline.on(Events.ROOM_WAITLIST_LOCK, (data) =>
      this._onWaitlistLock(data),
    );
    this._pipeline.on(Events.ROOM_WAITLIST_CYCLE, (data) =>
      this._onWaitlistCycle(data),
    );
    this._pipeline.on(Events.ROOM_TIME_SYNC, (data) =>
      this._onRoomTimeSync(data),
    );
    ...
    this._pipeline.on(Events.ROOM_CHAT_DELETE, (data) =>
      this._onChatDelete(data),
    );
    this._pipeline.on(Events.ROOM_CHAT_UPDATE, (data) =>
      this._onChatUpdate(data),
    );
    ...
    this._pipeline.on(Events.ROOM_USER_MUTE, (data) =>
      this._onRoomUserMute(data),
    );
    this._pipeline.on(Events.ROOM_USER_UNMUTE, (data) =>
      this._onRoomUserUnmute(data),
    );
    this._pipeline.on(Events.ROOM_USER_ROLE_UPDATE, (data) =>
      this._onRoomUserRoleUpdate(data),
    );
    this._pipeline.on(Events.ROOM_USER_AVATAR_UPDATE, (data) =>
      this._onRoomUserAvatarUpdate(data),
    );
    this._pipeline.on(Events.ROOM_USER_SUBSCRIPTION_UPDATE, (data) =>
      this._onRoomUserSubscriptionUpdate(data),
    );
    this._pipeline.on(Events.SYSTEM_MESSAGE, (data) =>
      this._onSystemMessage(data),
    );
    this._pipeline.on(Events.MAINTENANCE, (data) =>
      this._onMaintenance(data),
    );
    this._pipeline.on(Events.RATE_LIMIT, (data) =>
      this._onRateLimit(data),
    );
  ...
  _onWaitlistJoin(data) {
    ...
    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_WAITLIST_JOIN, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }
  ...
  _onWaitlistLock(data) {
    if (typeof data?.locked === "boolean") {
      this._waitlistLocked = data.locked;
    } else if (typeof data?.waitlistLocked === "boolean") {
      this._waitlistLocked = data.waitlistLocked;
    }

    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_WAITLIST_LOCK, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }

  _onWaitlistCycle(data) {
    if (typeof data?.enabled === "boolean") {
      this._waitlistCycleEnabled = data.enabled;
    } else if (typeof data?.waitlistCycleEnabled === "boolean") {
      this._waitlistCycleEnabled = data.waitlistCycleEnabled;
    }

    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_WAITLIST_CYCLE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }

  _onRoomTimeSync(data) {
    const serverNow = Number(
      data?.serverTime ?? data?.server_time ?? data?.timestamp ?? Date.now(),
    );
    if (Number.isFinite(serverNow)) {
      this._serverTimeOffsetMs = serverNow - Date.now();
    }

    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_TIME_SYNC, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }
  ...
  _onRoomUserRoleUpdate(data) {
    ...
    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_USER_ROLE_UPDATE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }

  _onRoomUserMute(data) {
    const uid = String(data?.userId ?? data?.user_id ?? data?.id ?? "");
    const existing = uid ? this._roomUsers.get(uid) : null;
    if (existing) existing.muted = true;

    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_USER_MUTE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }

  _onRoomUserUnmute(data) {
    const uid = String(data?.userId ?? data?.user_id ?? data?.id ?? "");
    const existing = uid ? this._roomUsers.get(uid) : null;
    if (existing) existing.muted = false;

    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_USER_UNMUTE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }

  _onRoomUserAvatarUpdate(data) {
    const uid = String(data?.userId ?? data?.user_id ?? data?.id ?? "");
    const existing = uid ? this._roomUsers.get(uid) : null;
    if (existing) {
      existing.avatarId = data?.avatarId ?? data?.avatar_id ?? null;
    }

    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_USER_AVATAR_UPDATE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }

  _onRoomUserSubscriptionUpdate(data) {
    const uid = String(data?.userId ?? data?.user_id ?? data?.id ?? "");
    const existing = uid ? this._roomUsers.get(uid) : null;
    if (existing) {
      existing.subscriptionType =
        data?.subscriptionType ?? data?.subscription_type ?? null;
      existing.subscriptionMonths =
        data?.subscriptionMonths ?? data?.subscription_months ?? null;
    }
  ...
  _onChatMessage(data) {
    ...
    if (content.startsWith(prefix)) {
      ...
      if (name) {
        ...
        if (messageId && this.cfg.deleteCommandMessagesEnabled) {
          this.scheduleMessageDelete(
            messageId,
            this.cfg.deleteCommandMessagesDelayMs,
          );
        }
      }
      return;
  ...
  _onChatDelete(data) {
    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_CHAT_DELETE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }
  ...
  _onChatUpdate(data) {
    if (!this._paused) {
      this.events
        .dispatch(Events.ROOM_CHAT_UPDATE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }

  _onSystemMessage(data) {
    this._lastSystemMessage =
      data?.message ?? data?.content ?? data?.text ?? null;
    this._log(
      "info",
      `[system] ${this._lastSystemMessage ?? JSON.stringify(data)}`,
    );

    if (!this._paused) {
      this.events
        .dispatch(Events.SYSTEM_MESSAGE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }
  ...
  _onMaintenance(data) {
    ...
    if (!this._paused) {
      this.events
        .dispatch(Events.MAINTENANCE, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }
  ...
  _onRateLimit(data) {
    this._rateLimitInfo = {
      retryAfterMs: Number(data?.retryAfterMs ?? data?.retry_after_ms ?? 0) || null,
    ...
    if (!this._paused) {
      this.events
        .dispatch(Events.RATE_LIMIT, this._buildEventCtx(), data)
        .catch(() => {});
    }
  }
  ...
  async sendChat(content) {
    const res = await this._api.chat.sendMessage(this.cfg.room, { content });
    const payload = res?.data?.data?.message ?? res?.data?.message ?? res?.data ?? null;
    const messageId = payload?.id ?? payload?.messageId ?? payload?.message_id ?? null;
    return { response: res, messageId };
  }

  scheduleMessageDelete(messageId, delayMs = 0) {
    const id = String(messageId ?? "").trim();
    if (!id || !this._api?.chat?.deleteMessage) return false;

    const delay = Math.max(0, Number(delayMs) || 0);
    const timer = setTimeout(async () => {
      this._deleteTimers.delete(timer);
      try {
        await this._api.chat.deleteMessage(this.cfg.room, id);
      } catch (err) {
        this._log("warn", `Delete message failed (${id}): ${err.message}`);
      }
    }, delay);

    this._deleteTimers.add(timer);
    return true;
  }
  ...
  async _maybeSendIntervalMessage() {
    ...
    if (this.cfg.motdEnabled) {
      ...
      if (interval > 0 && this._songCount % interval === 0) {
   ...
   */
  _buildCtx({ sender, args, rawArgs, message, messageId = null }) {
  ...
  getSessionState() {
    ...
    return {
      ...
      waitlistLocked: this._waitlistLocked,
      waitlistCycleEnabled: this._waitlistCycleEnabled,
      serverTimeOffsetMs: this._serverTimeOffsetMs,
      lastSystemMessage: this._lastSystemMessage,
      maintenance: this._maintenance,
      rateLimitInfo: this._rateLimitInfo,
    };
  }
}
